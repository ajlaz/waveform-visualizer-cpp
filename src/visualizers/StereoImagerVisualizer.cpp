...
void StereoImagerVisualizer::update(const AnalysisFrame &frame)
{
    ...
    for (size_t i = 0; i < n; ++i)
    {
        ...
        if (xf < 0.0f || xf >= texSize_ - 1 || yf < 0.0f || yf >= texSize_ - 1)
            continue;
...
{
    if (key == "decay")
        decay_ = std::clamp(value, 0.80f, 0.99f);
    else if (key == "brightness")
        brightness_ = std::clamp(value, 0.01f, 0.50f);
    else if (key == "scale")
        scale_ = std::clamp(value, 0.10f, 2.00f);
    else if (key == "edge_soften")
        edgeSoften_ = std::clamp(value, 0.0f, 1.0f);
}
